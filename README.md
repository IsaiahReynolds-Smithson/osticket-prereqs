...
We will use Microsoft Remote Connect Protocol to connect to our Virtual Machine using the VM's `public IP address`. 
...
On the local machine search for 'Remote Desktop Connection' and the following window opens, you can now copy and paste the `public IP address` into the provided space. Then press the `Connect` button. 
